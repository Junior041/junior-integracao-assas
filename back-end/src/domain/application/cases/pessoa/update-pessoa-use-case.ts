...
export class UpdatePessoaUseCase {
  ...
  async execute({ idPessoa, nome }: UpdatePessoaUseCaseRequest): Promise<UpdatePessoaUseCaseResponse> {
    ...
    const pessoaAtualizada = await this.pessoaRepository.save(new UniqueEntityID(idPessoa), pessoa);
