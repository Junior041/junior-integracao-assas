...
type DeletePessoaUseCaseResponse = Either<
...
export class DeletePessoaUseCase {
  ...
  async execute({
    idPessoa,
  }: DeletePessoaUseCaseRequest): Promise<DeletePessoaUseCaseResponse> {
    ...
    if (pessoa.enderecos && pessoa.enderecos.length > 0) {
      return left(
        new GenericoErro(
          'Pessoa possuí endereços ativos. não é possível deletar.',
        ),
      );
    }
    ...
    if (usuario) {
      return left(
        new GenericoErro('Não é possível deletar, pois a pessoa é um USUARIO.'),
      );
    }
