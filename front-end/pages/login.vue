...
const login = () => {
  if (email.value === 'teste@teste.com' && password.value === '123') {
...
<template>
  <v-container fluid class="d-flex align-center justify-center fill-height bg-primary" style="min-height: 100vh;">
    <v-card class="pa-6 rounded-xl shadow-xl bg-white" max-width="400" width="90%">
      <v-card-title class="text-center text-h5 font-bold text-primary-dark">Bem-vindo!</v-card-title>
      <v-card-subtitle class="text-center text-secondary-light">Faça login para continuar</v-card-subtitle>
      ...
      <v-form class="mt-4" @submit.prevent="login">
        ...
        <v-alert
          ...
          variant="tonal"
          density="compact"
          class="mb-2 text-caption text-center"
        >
          {{ errorMessage }}
        </v-alert>
